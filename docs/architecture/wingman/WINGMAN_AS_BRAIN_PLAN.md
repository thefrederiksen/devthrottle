# SessionWingman as the Brain — Implementation Plan (Phase 4)
...
Phase 3 made `Session.StatusColor` a first-class field and gave it a per-Director wingman. That fixed the "who owns the color" question but left three real holes:

1. **The wingman lies on the most common case.** `ActivityState=WaitingForInput` collapses two very different situations: "agent asked you a question mid-task" (genuinely needs you) vs. "agent finished its turn and is back at the idle prompt" (done, ready for next task). The Phase 3 fast path paints both red. So a session that just successfully answered "why is the sun yellow" sits at the prompt and the dot says RED. That trains the user to ignore red.

2. **Desktop and Gateway disagree.** The desktop sidebar paints a static per-session "identity color" (`Session.CustomColor`) that the user picked, which has nothing to do with status. The Gateway paints the wingman color. So the same session shows green in one window and red in another. Color is supposed to mean "attention required"; right now it means two different things in two places.

3. **The wingman is a black box.** You can't see what it decided, why, when, or what the last turn summary said. If the dot is wrong (as in case #1) you have no way to debug from the UI. The original Wingman spec promised an Agent View and a Voice View; both exist as embryos but they're separate and neither shows the wingman's reasoning.
...
2. **The SessionWingman is the sole source of color.** Desktop and Gateway both read `Session.StatusColor` from the same field. No second path, no static override.
3. **No fallbacks. No UI-side derivation.** If the wingman hasn't decided yet, the UI shows what the wingman wrote (default green). It never invents.
4. **The wingman must be observable.** Every decision lands in a per-session event log readable from the API. If the user can see the dot, they can see why.
5. **The wingman reduces cognitive load — that's the job.** When a session needs the user, the wingman produces a one-sentence distilled question. The merged Session View shows that, not the raw 800-word agent blob.

---

## Decisions recorded

- **`WaitingForInput` is no longer red by default.** The fast path treats it as Green ("ready, awaiting next prompt"). Red only when the wingman has positive evidence of a pending question (either a turn-summary `needs_user != "no"` OR a buffer-scan heuristic for question markers like `❯ Do you want to`).
...
- **Wingman observability ships as `GET /sessions/{sid}/wingman`** — a single endpoint that returns current color, current reason, last N transitions (timestamped), and the latest turn summary.

---

## Out of scope (Phase 5+)

- Wingman-initiated actions ("auto-summarize commits before push", "auto-run review-code before commit"). Phase 4 is read-only and notification-only.
- Cross-session wingman reasoning ("you have 3 reds in the same repo").
- Persisting the wingman event log across Director restarts. In-memory ring buffer is fine for v1.
...
**Touches:**
- `src/CcDirector.Core/Wingman/SessionStatusWingman.cs` — `ColorFromActivityState`:
...
- `TurnSummaryCache` slow path (`ApplyTurnSummary`) — unchanged in shape, but the wingman's slow-path rules are the canonical path that promotes a session to Red after a turn ends with `needs_user != "no"`.
...
**Tests:**
- Existing tests update: `WaitingForInput_maps_to_red` becomes `WaitingForInput_maps_to_green_until_wingman_promotes`.
...
### 4b — Wingman observability

**Why:** Make the wingman's reasoning visible. If the dot is wrong, the user can see why and we can fix it.

**Touches:**
- `src/CcDirector.Core/Sessions/Session.cs` — add `WingmanEventLog` (ring buffer, capacity 50). Every `SetStatusColor` writes `{ts, oldColor, newColor, reason, source}` where `source ∈ {"activity", "turn-summary", "promote", "init"}`.
- `src/CcDirector.Gateway.Contracts/WingmanViewDto.cs` (NEW) — `{ currentColor, currentReason, since, events[], latestTurnSummary }`.
- `src/CcDirector.ControlApi/ControlEndpoints.cs` — new `GET /sessions/{sid}/wingman` returns the DTO.
- `src/CcDirector.Gateway/Api/GatewayEndpoints.cs` — new `GET /sessions/{sid}/wingman` forwards to owning Director (read-through proxy).

**Tests:**
- `WingmanEventLog_records_each_color_change`
- `WingmanEventLog_ring_buffer_evicts_oldest_past_capacity`
- `GET /sessions/{sid}/wingman` returns the latest entry first
...
**Why:** One view to live in. Replaces Raw as the default. Shows the wingman's reasoning, the wingman's distilled question, the turn summary, voice playback — everything you need to triage and respond without reading the terminal blob.

**Web side (Gateway `Web/`):**
- New: `Web/session.html` (or update existing `manager.html` / Director's per-session view) — single tab layout:
  - Top: wingman banner (color dot, distilled reason, ago-time)
  ...
  - Bottom: wingman event log (collapsed by default)
...
**Tests:**
- Web smoke: `/sessions/{sid}/view` renders the Session layout, queries wingman + turn summary.
- Avalonia: covered by manual smoke (no UI unit tests on Avalonia today).

### 4d — Desktop reads wingman color, drops static session color from sidebar
...
- Subscribe to `Session.OnStatusColorChanged` to repaint when the wingman moves the color.
...
### 4e — Wingman's "distilled question" prompt

**Why:** When the wingman says red, the user wants one crisp sentence — not 800 words of raw agent output. The wingman's job is to be the editor.

**Touches:**
- `src/CcDirector.Core/Wingman/WingmanService.cs` — `BuildTurnSummaryPrompt` extends the JSON shape:
  ```json
  {
    "headline": "...",
...
- `SessionStatusWingman.ApplyTurnSummary` uses `NeedsUserShort` (falls back to `NeedsUserDetail` if empty) as the `LastStatusReason` when promoting to red.
...
| File | Change |
|---|---|
| `src/CcDirector.Core/Wingman/SessionStatusWingman.cs` | EDIT — `WaitingForInput` -> Green; add `PromotePendingQuestion`; buffer-marker heuristic on activity-state change |
| `src/CcDirector.Core/Wingman/WingmanService.cs` | EDIT — extend turn-summary prompt + parser for `needs_user_short` |
| `src/CcDirector.Gateway.Contracts/TurnSummary.cs` | EDIT — add `NeedsUserShort` |
| `src/CcDirector.Core/Sessions/Session.cs` | EDIT — `WingmanEventLog` ring buffer; record each color change |
| `src/CcDirector.Gateway.Contracts/WingmanViewDto.cs` | NEW — `{ currentColor, currentReason, since, events[], latestTurnSummary }` |
| `src/CcDirector.ControlApi/ControlEndpoints.cs` | EDIT — new `GET /sessions/{sid}/wingman` |
| `src/CcDirector.Gateway/Api/GatewayEndpoints.cs` | EDIT — forward `GET /sessions/{sid}/wingman` |
| `src/CcDirector.Gateway/Discovery/DirectorEndpointClient.cs` | EDIT — `GetWingmanAsync` |
...
| `src/CcDirector.Core.Tests/Wingman/SessionStatusWingmanTests.cs` | EDIT — new disambiguation tests |
| `src/CcDirector.Core.Tests/Wingman/WingmanEventLogTests.cs` | NEW |
| `src/CcDirector.Gateway.Tests/WingmanAggregationTests.cs` | NEW — endpoint forwarding + stamping |
| `docs/architecture/wingman/WINGMAN_AS_BRAIN_PLAN.md` | NEW — this doc |
...
2. When a Claude session asks a real question mid-turn (`needs_user != "no"`), the dot turns red within ~2s and the wingman's `LastStatusReason` is the distilled one-sentence question.
...
4. `GET /sessions/{sid}/wingman` returns current color, reason, last N events (with timestamps), and latest turn summary. Same endpoint works on the Gateway via the read-through proxy.
...
2. **Voice TTS auto-play on red.** Today voice is manual. Should the wingman auto-speak the distilled question when a session moves into red, if voice mode is on? Recommend: yes, behind a per-session toggle, default on.
