...
| Target architecture (hands/brain/face) | [`DIRECTOR_DUMB_WRAPPER_TARGET.md`](DIRECTOR_DUMB_WRAPPER_TARGET.md) | PLANNED | The vision and end-state split: Director = hands + permanent desktop fallback (Terminal + Source Control, any agent), Gateway = brain (all interpretation/judgment/orchestration, the promoted decision-support Wingman), Cockpit = face (primary UI, per-session + fleet voice / car mode). Trust ladder (reliability -> intelligence -> voice), boundary rule, violation inventory, Facts/Events/Verbs contract, migration tracks A/B/C, standing vetoes. |
