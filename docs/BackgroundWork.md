...
| Three bounded waits inside a session: for a fresh transcript after a context clear, for a compaction to finish, and, for a pre-prompt to an agent the first-prompt gate cannot read, for the terminal to settle before the pre-prompt is typed | `src/CcDirector.Core/Sessions/Session.cs` | 3 | A context clear; a compact; a create with a pre-prompt | The transcript wait in 250 ms polls to a deadline; the compaction wait on its own poll interval to its own timeout; 750 ms polls until the terminal is quiet, to the pre-prompt wait | A bounded wait | One transcript check; one driver query; one activity and buffer read | Pool | A fresh transcript, a finished compaction or a settled terminal, the deadline, or cancellation | - |
