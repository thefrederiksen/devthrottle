...
| Four bounded waits inside a session: for a fresh transcript after a context clear, for a compaction to finish, for the terminal to settle before a pre-prompt to an agent the first-prompt gate cannot read is typed, and, for a send the agent's records cannot prove, for its text to leave the composer after the Enter | `src/CcDirector.Core/Sessions/Session.cs` | 4 | A context clear; a compact; a create with a pre-prompt; a send the records cannot prove | The transcript wait in 250 ms polls to a deadline; the compaction wait on its own poll interval to its own timeout; 750 ms polls until the terminal is quiet, to the pre-prompt wait; 250 ms polls of the composer, to a ten-second window | A bounded wait | One transcript check; one driver query; one activity and buffer read; one composer read | Pool | A fresh transcript, a finished compaction, a settled terminal or an emptied composer, the deadline, or cancellation | - |
