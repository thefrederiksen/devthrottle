...
| Five bounded waits inside a session: for a fresh transcript after a context clear, for a compaction to finish, for the terminal to settle before a pre-prompt to an agent the first-prompt gate cannot read is typed, for a send's text to leave the composer after the Enter, and - after a send to a working agent returned still delivering - for its prompt to reach the agent's records late (read only; it types nothing) | `src/CcDirector.Core/Sessions/Session.cs` | 5 | A context clear; a compact; a create with a pre-prompt; a send the records cannot prove, or a records miss while the agent works; a send that returned still delivering | The transcript wait in 250 ms polls to a deadline; the compaction wait on its own poll interval to its own timeout; 750 ms polls until the terminal is quiet, to the pre-prompt wait; 250 ms polls of the composer, to a ten-second window; 250 ms reads of the records, to a 15-minute limit | A bounded wait | One transcript check; one driver query; one activity and buffer read; one composer read; one read of the new lines of the agent's records | Pool | A fresh transcript, a finished compaction, a settled terminal, an emptied composer or the prompt in the records, the deadline, the session ending, or cancellation | - |
