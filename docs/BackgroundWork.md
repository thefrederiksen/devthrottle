...
| GitHub Actions backend run watch, and its bounded wait | `src/CcDirector.Core/Backends/GitHubActionsBackend.cs` | 2 | Polling loop while a run is watched | While a run is in flight | While a process runs (a remote run) | One status request per poll | Pool | Run ends or cancellation | 0 today |
...
| Wait for a stopped session's process to go, and a bounded wait in the stop path | `src/CcDirector.ControlApi/SessionCommandExecutor.cs` | 2 | A stop command | Process exit poll interval until the settle window ends; 750 ms polls to a deadline | A bounded wait | One liveness read | Pool | The process is gone, or the window ends | - |
| Wait for the terminal to go quiet before an ask is typed, and a second bounded wait in the same runner | `src/CcDirector.Core/Drivers/SessionAskRunner.cs` | 2 | A session ask | Poll interval until quiet, or the ask's timeout | A bounded wait | Reads the buffer clock | Pool | Quiet, the timeout, or cancellation | - |
...
| The drain's waits: for sessions to flag their handovers, for them to close, and one more loop the scan counts | `src/CcDirector.ControlApi/Drain/DirectorDrain.cs` | 3 | A smart shutdown | The drain's poll interval, through an injected delay, until the deadline | A bounded wait | One roster read per poll | Pool | The seats are done, the deadline, or cancellation | - |
...
| Two bounded waits inside a session: for a state to settle, and for input to be accepted | `src/CcDirector.Core/Sessions/Session.cs` | 2 | A session action | 250 ms polls to a deadline | A bounded wait | One state read | Pool | The condition, or the deadline | - |
...
| Wait until the Director's install target can be replaced | `tools/cc-director-setup-engine/DirectorUpdateApply.cs` | 2 | A Director update | 500 ms sleeps to a deadline | A bounded wait | One file check | The installer's thread | Replaceable, or the deadline | - |
| Wait until the Gateway's files are writable during its self-update | `tools/cc-director-setup-engine/GatewaySelfUpdate.cs` | 2 | A Gateway self-update | 500 ms sleeps to a deadline | A bounded wait | One file check | The installer's thread | Writable, or the deadline | - |
...
| Wait until the launcher's files are writable during its self-update | `tools/cc-director-setup-engine/LauncherSelfUpdate.cs` | 2 | A launcher self-update | 500 ms sleeps to a deadline | A bounded wait | One file check | The installer's thread | Writable, or the deadline | - |
...
| Shutdown signal listeners: the macOS and Linux request-file poll, and the Windows named-event pump | `src/CcDirector.Core/Lifecycle/LifecycleSignal.cs` | 2 | Polling loop on a request file, one thread per signal; on Windows a thread blocked in `WaitAny` on the kernel event | Every 500 ms on macOS and Linux; instantaneous, no polling, on Windows | Slow and steady; **left as is, deliberately** - the code records a measured 1-in-5 lost event with a file watcher, and a missed shutdown is worse than a half-second-late one. Owner's decision (report 1, question 4) | One `File.Exists` | Its own thread | Listener disposed | About 4 stat calls a second |
