...
| Provider usage poll and credentials watcher | `src/CcDirector.Core/Claude/ClaudeUsageService.cs` | 2 | Timer; a file watcher on the credentials file | Every 60 s | Never on a timer; being deleted by #3318 - nothing read its result | One web request per stored account | Pool | Service disposed | 1,882 log lines; 0 after #3318 (remove this row when it merges) |
