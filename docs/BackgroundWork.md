...
| Job | Source | Sites | Trigger today | Cadence today | Tier | One run costs | Thread | Off switch | Today on this Mac |
|---|---|---|---|---|---|---|---|---|---|
| Repository watcher: a root-folder watcher, a per-repository watcher, and the 5-minute full reconciliation | `src/CcDirector.Core/Git/RepositoryWatcher.cs` | 2 | A file event under a root or a repository; the reconciliation is a job on the scheduler, "Repository watcher: full reconciliation" | Recompute 2 s after a change; full rescan every 5 min unconditionally | On change (the rescan is slow-and-steady, **on the scheduler**, and should only run after a dropped event) | One recompute: the whole probe set for one repository, about 186 git commands on a 28-worktree repository, plus a fleet-list download | Pool | Never while the Director runs | 4,336 recomputes of one repository; fixed by #3298 (its own index writes no longer count) |
| Per-session uncommitted count | `src/CcDirector.Core/Git/SessionGitStatusMonitor.cs` | 0 | A job on the scheduler, "Per-session uncommitted count" | Every 15 s, 4 repositories at a time; its own cache lasts 10 s so it is always cold | Slow and steady; proposed: drive it from the watcher, or cache longer than the poll | One `git status --porcelain` per distinct repository | Pool | Stops with the session manager | 1,132 log lines |

### Gateway and network

| Job | Source | Sites | Trigger today | Cadence today | Tier | One run costs | Thread | Off switch | Today on this Mac |
|---|---|---|---|---|---|---|---|---|---|
| Tunnel re-push of the full snapshot, and the reconnect loop that re-dials the Gateway after a drop | `src/CcDirector.ControlApi/GatewayStreamClient.cs` | 1 | A job on the scheduler, "Tunnel re-push of the full snapshot"; a loop that runs while the client is not disposed, dialling, waiting for the connection to close, delaying, dialling again | Every 10 s, changed or not; the reconnect waits its restart delay between dials | Slow and steady, **on the scheduler**; proposed: send on change, a small keep-alive otherwise (#3301) | Full session list, a key registration per session, the full repository snapshot | Pool | Stream client disposed | 16,260 log lines; the "tick LATE" line is now written only for a second or more (#3315) |
| Repository snapshot push debounce, and the injected-text, workflow-index, skill-index and skill-store refresh cycle | `src/CcDirector.ControlApi/ControlApiHost.cs` | 1 | A repository change (debounced); the refresh cycle is a job on the scheduler, "Skills, workflows and injected text refresh cycle" | Debounce once per burst; the refresh cycle every 60 s | Once-then-stop; slow and steady - the skill store now downloads only a changed version (#3310). **The cycle is on the scheduler**; proposed for the three indexes: 5 min with a not-modified check | Three index downloads; the skill bodies only when the served version or hash changed | Pool | Host disposed | 960 refresh cycles; 15 skill directories rewritten per cycle before #3310, 0 after |
| Turn sweep | `src/CcDirector.ControlApi/TurnPusher.cs` | 0 | A job on the scheduler, "Turn sweep" | Every 1 min | Slow and steady; keep. **On the scheduler** | Push new turns; cheap when empty | Pool | Pusher disposed | Not separately counted |
...
| Instance registration heartbeat | `src/CcDirector.ControlApi/InstanceRegistration.cs` | 0 | A job on the scheduler, "Instance registration heartbeat" | Every 15 s; re-writes the file only when it is missing | Slow and steady; keep. **On the scheduler** | One small request | Pool | Registration disposed | Not separately counted |
...
| Error report send loop: sends the errors this process logged to its Gateway, in both the Director and the launcher (#3311) | `src/CcDirector.Core/ErrorReports/ErrorReporter.cs` | 1 | A periodic timer started once at process start by `ErrorReporter.Start`; errors are queued in memory by the log observer, never sent from the logging thread | Every 20 s; a tick with nothing pending, inside a pause, or over the budget takes one lock and sends nothing. At most 60 reports an hour; a pause of 10 min after the Gateway refuses a batch and 1 h after it has no route | Slow and steady; keep. A batch holds at most 25 reports, a failed batch is tried at most 3 times, the queue holds at most 200 distinct errors | One POST of at most one batch to `/gateway/director-errors`, only when something is pending and a Gateway credential exists | Pool | Never while the process runs: only `Dispose` stops it, and only tests call that. A machine with no Gateway credential still ticks but sends nothing | New on 23 September 2026; not yet measured |

### Sessions, the terminal and the wingman

| Job | Source | Sites | Trigger today | Cadence today | Tier | One run costs | Thread | Off switch | Today on this Mac |
|---|---|---|---|---|---|---|---|---|---|
| Terminal buffer poll | `src/CcDirector.Terminal.Avalonia/TerminalControl.cs` | 1 | Render-priority timer (one construction site, one control) | 20 times a second while the control exists; stopped while a buffer replay runs on a pool thread and started at its handover | On view; keep the rate, do far less per tick (Stage 2, another session's pull requests #3305 and #3307) | Read new bytes, parse, repaint | Window | Control detached | Not logged |
| Deletion reaper | `src/CcDirector.Core/Sessions/SessionManager.cs` | 0 | A job on the scheduler, "Deletion reaper", one per session manager | Every 30 s | Slow and steady; keep. **On the scheduler** | A directory enumeration; cheap when empty | Pool | Manager disposed | Not separately counted |
| Session pointer watcher and sweep | `src/CcDirector.Core/Sessions/SessionPointerWatcher.cs` | 1 | A file watcher, and beside it a job on the scheduler, "Session pointer sweep" | Sweep every 2 s | On change, with the sweep as the guarantee; keep. **The sweep is on the scheduler** | A directory enumeration | Pool | Watcher disposed | Not separately counted |
...
| GitHub Actions backend run watch, and the worker thread's wait for its first turn to be established | `src/CcDirector.Core/Backends/GitHubActionsBackend.cs` | 2 | Polling loop while a run is watched; a first-turn wait when the worker starts | While a run is in flight; the first-turn wait in 250 ms polls with a thirty-second ceiling | While a process runs (a remote run); the first-turn wait is a bounded wait | One status request per poll for the run watch; one field read per poll for the first-turn wait | Pool | Run ends or cancellation; the thread is established or the thirty-second ceiling | 0 today |
...
| A bounded wait in the command executor for a stopped session's process to go (the pre-prompt wait moved into `Session.DeliverPrePromptAsync`) | `src/CcDirector.ControlApi/SessionCommandExecutor.cs` | 1 | A stop command | Process exit poll interval until the settle window ends | A bounded wait | One liveness read | Pool | The process is gone, or the window ends | - |
| Wait for the terminal to go quiet before an ask is typed, and wait for the reply to stabilise afterwards | `src/CcDirector.Core/Drivers/SessionAskRunner.cs` | 2 | A session ask | Poll interval until quiet, or the ask's timeout; then polls until the reply stops changing | A bounded wait | Reads the buffer clock; reads the transcript | Pool | Quiet, the timeout, or cancellation | - |
...
| Three bounded waits inside a submit: for the terminal to echo a typed line, for the terminal to go quiet before a retained composer is cleared, and for an agent to take in a bracketed paste before the Enter | `src/CcDirector.Core/Drivers/TerminalSubmit.cs` | 3 | A submit | The echo wait polls to a deadline that runs from the last sign of progress, capped at 60 s; the quiet wait in 100 ms polls to 15 s; the paste wait in 50 ms polls to 30 s | A bounded wait | One buffer read, and a screen read every 500 ms during the echo wait | Pool | The echo, the quiet, or the deadline | - |
| Five bounded waits inside a session: for a fresh transcript after a context clear, for a compaction to finish, for the terminal to settle before a pre-prompt to an agent the first-prompt gate cannot read is typed, for a send's text to leave the composer after the Enter, and - after a send to a working agent returned still delivering - for its prompt to reach the agent's records late, or with no records to watch or after a failed read of them, for the same limit to end (read only; it types nothing; it always ends with an answer, so nothing stays delivering forever) | `src/CcDirector.Core/Sessions/Session.cs` | 5 | A context clear; a compact; a create with a pre-prompt; a send the records cannot prove, or a records miss while the agent works; a send that returned still delivering | The transcript wait in 250 ms polls to a deadline; the compaction wait on its own poll interval to its own timeout; 750 ms polls until the terminal is quiet, to the pre-prompt wait; 250 ms polls of the composer, to a ten-second window; 250 ms reads of the records, to a 15-minute limit | A bounded wait | One transcript check; one driver query; one activity and buffer read; one composer read; one read of the new lines of the agent's records (none with no records to watch, or after a read failed) | Pool | A fresh transcript, a finished compaction, a settled terminal, an emptied composer or the prompt in the records, the deadline, the session ending, or cancellation | - |
