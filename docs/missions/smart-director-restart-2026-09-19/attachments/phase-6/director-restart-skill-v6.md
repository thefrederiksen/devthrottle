...
**And it offers them back.** Merged 20 September 2026 (pull request 3208). Once the restarted Director
reaches its Gateway it asks the way up engine ONCE; if a record still owes seats a window opens,
headlined **"A restart is available"** - when, the reason, how many seats are owed, and a ticked list of
mission heads with their seats under them, plus one row per seat that ended without a handover. Two
answers: **Bring back** and **Not now**. A seat with no handover is never brought back with the rest; it
is offered a reopen instead - "Reopen its saved conversation" for an agent whose plugin declares
`CanResumeSavedConversation`, and "Open a fresh session in its repository" for one that does not.
**File, Restart history...** lists every record, newest first, and carries the same offer for any record
still owed, so "Not now" is not final. With the Gateway silent nothing is shown at start-up at all - the
reason goes to the log - and the history says the records could not be read rather than showing none.
...
### 1. Bring the sessions back from somewhere other than that Director's screen

The Director offers them itself now, at start-up and in File, Restart history (above). **Both offers are
windows on that Director's own screen**, so they are no use to you when you are not sitting at it - and
there is still no command-line door to the smart restart or to the history.

When you have to do it from here, the restore is the way, and it is unchanged:
...
**Do not race the owner's window.** If that Director has just started, its own offer may be open in
front of him, and the engine refuses a second bring back of a seat it has already started - but the
cheaper answer is to ask whether it has been answered before reaching for the command line.
...
- **Codex has no conversation resume.** Its driver ignores the id, so a Codex session ended at the
  limit comes back blank, in its own repository, with nothing of its conversation. The offer says so
  in those words before you press it, so this is a thing to expect rather than a thing to work round.
...
The dialog has an "Answer these first?" section listing sessions with a question box open for the owner.
It is filled now (merged 20 September 2026, pull request 3215): `PendingInteractionWatcher` reads each
session's own transcript at every turn end and marks a Claude Code seat holding an unanswered
`AskUserQuestion` or `ExitPlanMode`. **It sees only that.** No other agent is inspected, and a permission
ask is never shown, because its source was a hook event and a hook event is not in the transcript. So the
section is real but partial: look over the fleet yourself as well before starting, because a session
waiting on an answer cannot hand over while the box holds it.
