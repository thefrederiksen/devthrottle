...
<#
.SYNOPSIS
    The isolated rig for the phase 3b proof (issue #3025, child of #2936): its own storage root, its own
    Gateway, its own launcher, its own Director, one live session on it and one published sample report,
    all built from this worktree. One command stands the whole thing up: .ig.ps1 all
...
.PARAMETER Command
    ...
    report        publish sample-report.html (or -ReportFile) as that session with this worktree's
                  cc-dev-reports, and print the one owner address it returns
    all           build, up, session, report - the whole rig in one command
...
param(
    ...
    [ValidateSet('build', 'up', 'session', 'report', 'all', 'stage-shells', 'status', 'down', 'reset')]
    [string]$Command,
    [string]$Root = (Join-Path $env:LOCALAPPDATA "dev-report-proof-rig-p3b"),
    [int]$GatewayPort = 7951,
    [string]$Builds = (Join-Path $env:TEMP "dev-report-proof-rig-p3b-builds"),
    [int]$Slot = 5,
    [string]$ShellsFrom = '',
    [string]$ReportFile = '',
    [switch]$Force
...
function Say([string]$text) { Write-Host "[p3b-proof-rig] $text" }
function Fail([string]$text) { throw "[p3b-proof-rig] $text" }
...
$reportFileOut = Join-Path $Root "report.json"
$gatewayUrl    = "http://127.0.0.1:$GatewayPort"
$machine       = $env:COMPUTERNAME
$taskGateway   = "dev-report-proof-rig-p3b-gateway"
$taskLauncher  = "dev-report-proof-rig-p3b-launcher"
...
function Invoke-Session {
    ...
    $prePrompt = "You are a test fixture for an automated proof. Run this exact command with your shell tool: " +
        ...
        "tool; answer with the single word: received. The ONE exception: a message whose first line is exactly " +
        "FIXTURE-COMMAND is an instruction from the proof itself - run the shell command on the lines after it, " +
        "exactly as written, and then answer with its output and nothing else."
    $body = @{
        RepoPath          = $repo
...
function Invoke-Report {
    if (-not (Test-Path $sessionFile)) { Fail "no $sessionFile - run 'session' first." }
    $s = Get-Content $sessionFile -Raw | ConvertFrom-Json
    $file = if ($ReportFile) { [System.IO.Path]::GetFullPath($ReportFile) } else { Join-Path $here "sample-report.html" }
    if (-not (Test-Path $file)) { Fail "no report at $file" }

    $tool = Join-Path $repo "tools\cc-dev-reports\main.py"
    if (-not (Test-Path $tool)) { Fail "no cc-dev-reports in this worktree at $tool" }

    # Published AS the rig session, against the rig Gateway. The tool on PATH points at another
    # checkout, so this worktree's copy is called by path on purpose.
    $env:CC_GATEWAY_URL = $s.CC_GATEWAY_URL
    $env:CC_GATEWAY_SESSION_KEY = $s.CC_GATEWAY_SESSION_KEY
    $env:CC_SESSION_ID = $s.CC_SESSION_ID
    Say "publishing $file as session $($s.CC_SESSION_ID)"
    $out = & python $tool open $file --json
    if ($LASTEXITCODE -ne 0) { Fail "cc-dev-reports open failed: $out" }
    $parsed = ($out -join "`n") | ConvertFrom-Json
    if (-not $parsed.ok) { Fail "cc-dev-reports open refused it: $($out -join ' ')" }
    Set-Content -Path $reportFileOut -Value ($out -join "`n") -Encoding ascii
    Say "report id $($parsed.report.id) version $($parsed.report.version)"
    Say "OWNER ROUTE: $gatewayUrl$($parsed.ownerRoute)"
    Say "written to $reportFileOut"
}
...
switch ($Command) {
    ...
    'all'          { Invoke-Build; Invoke-Up; Invoke-Session; Invoke-Report }
