...
Settings (GET/PUT /settings), tools catalog (/tools), scheduler (/scheduler), workspaces (/workspaces, /history), screenshots (/screenshots), dictation and TTS (/dictate, /tts), wingman (/sessions/{sid}/wingman/...), and a WebSocket terminal stream (/sessions/{sid}/stream). Explore `GET /healthz` for the Director id and version.
