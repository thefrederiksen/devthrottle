...
/// gives back one <see cref="SmartShutdownResult"/>. It shuts nothing down itself.
///
...
public partial class SmartShutdownDialog : Window
{
    ...
    /// <summary>
    /// What the owner chose. It stays <see cref="SmartShutdownResult.Cancelled"/> until a button says
    /// otherwise, so closing the window by its own X is cancelled.
    /// </summary>
    public SmartShutdownResult Result { get; private set; } = SmartShutdownResult.Cancelled;

    /// <summary>Shows the dialog over <paramref name="owner"/> and returns what the owner chose.</summary>
    public async Task<SmartShutdownResult> ShowForResultAsync(Window owner)
    {
    ...
    private void BtnSmart_Click(object? sender, RoutedEventArgs e) =>
        CloseWith(nameof(BtnSmart_Click), () => ViewModel.BuildSmartShutdownResult());

    private void BtnIgnore_Click(object? sender, RoutedEventArgs e) =>
        CloseWith(nameof(BtnIgnore_Click), () => SmartShutdownResult.IgnoreAllSessions);

    private void BtnCancel_Click(object? sender, RoutedEventArgs e) =>
        CloseWith(nameof(BtnCancel_Click), () => SmartShutdownResult.Cancelled);
    ...
    private void CloseWith(string handler, Func<SmartShutdownResult> choose)
    {
        try
        {
            Result = choose();
