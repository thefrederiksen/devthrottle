...
internal sealed class HeldDeliveryDriver : IDisposable
{
    ...
    {
        var uid = VoiceUploadStore.NormalizeUploadId(uploadId)
            ?? throw new ArgumentException($"'{uploadId}' is not an upload id", nameof(uploadId));
        var now = Now();
    ...
    private async Task DriveOneAsync(Held held, string trigger)
    {
        ...
        try
        {
            ...
            using (_enterScope(held.Tenant))
            {
                ...
                result = held.Kind == HeldDeliveryKind.Dictation
    ...
    private string? SessionOf(Held held)
    {
        ...
        try
        {
            return held.Kind == HeldDeliveryKind.Dictation
                ? store.Read(held.UploadId).Record?.Owned?.SessionId
                : store.ReadSendAnyway(held.UploadId)?.SessionId;
        }
        catch (Exception ex)
        {
