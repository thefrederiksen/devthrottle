...
public sealed class GatewayHost : IAsyncDisposable
{
    ...
    public async Task StartAsync()
    {
        ...
        _fleetManagerEvents = new Fleet.FleetManagerEventService(_fleetManagerEventStore!,
            new Fleet.GatewayFleetManagerEventEnvironment(PushedSessions, _streamStaleAfter,
                ...
                verdictsShown: tenant => _tenantSettingsResolver.TurnVerdict(tenant).ColourEnabled,
        ...
        _turnEndWatcher = new TurnEndWatcher(
            onTurnEnd: signal =>
            {
                ...
                Task<Wingman.TurnVerdictOutcome>? verdict = null;
                try
                {
                    using (_tenantBoundary.EnterScope(tenant))
                        verdict = _turnVerdictService?.StartTurnEnd(signal);
                }
                catch (Exception ex)
                {
                ...
                // The Fleet Manager's events (step 4), the ONE hook: a stop of a session a Fleet Manager owns is
                // recorded once the reading above is stored, and a Fleet Manager's own turn end is where what it is
                // owed is delivered. Fire and forget; it never throws.
                _fleetManagerEvents?.OnTurnEnd(signal, verdict);
            ...
            onSessionExited: (tenant, sid, _) => _fleetManagerEvents?.OnSessionExited(tenant, sid));
    ...
    public async Task StopAsync()
    {
        ...
        // take writes cannot hold shutdown. If the drain runs out of time the writer is ABANDONED, so it stops writing
        // into a database that is being disposed; the traces it did not write are logged and counted by the writer.
        try
        {
            if (_turnVerdictService is not null
                && !await _turnVerdictService.WaitForFlightsAsync(TurnVerdictTraceDrainTimeout).ConfigureAwait(false))
                FileLog.Write($"[GatewayHost] verdict flights did not finish within {TurnVerdictTraceDrainTimeout.TotalSeconds:0}s; a trace one of them hands in later is refused and logged");
            ...
            {
                _turnVerdictTraceWriter.Abandon();
                FileLog.Write($"[GatewayHost] turn verdict traces did not finish writing within {TurnVerdictTraceDrainTimeout.TotalSeconds:0}s; the writer is abandoned and the traces still queued are logged, not written");
            }
        }
