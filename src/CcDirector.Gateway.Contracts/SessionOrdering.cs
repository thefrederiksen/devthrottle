...
public static class SessionOrdering
{
    ...
    public static string? SupervisorLostNotice(SessionDto s, SessionDto? supervisor)
    {
        ...
        // brief and hires the Manager (the mission workflow: cc-devthrottle workflow instructions mission),
        // and the resolver enforces the direction - Manager-derivation EXCLUDES Architect, so the session
        // on the other end of that controller id is a session that OPENED this one, never a seat this one
        // answers to. That is what makes losing it provenance rather than supervision.
        //
