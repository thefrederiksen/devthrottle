...
public static class WingmanService
{
    ...
    private static string BuildTerminalStatePrompt(string terminalTail, string agentName)
    {
        ...
        sb.AppendLine("- unknown: the screen is too garbled or sparse to tell - blank, random box-drawing/garbage characters, or only a startup banner with NO input box, NO spinner, and NO footer. When you cannot positively identify the state, return unknown. NEVER fabricate \"working\" from garbage with no active indicator.");
        sb.AppendLine();
        sb.AppendLine("Traps to avoid:");
        sb.AppendLine("- Do NOT report working just because there is a lot of text or a spinner verb sits in the SCROLLBACK; only a CURRENT active indicator (\"esc to interrupt\" / live spinner+elapsed at the bottom) means working.");
        sb.AppendLine("- Do NOT report waiting_for_input just because you see an empty input box, IF \"esc to interrupt\" is also on screen - that combination is working (see the tie-break above).");
        sb.AppendLine("- Do NOT report waiting_for_permission just because you see the word \"permission\". The mode footer (\"bypass permissions on\", \"accept edits on\", \"plan mode on\", \"shift+tab to cycle\") is a status line, not a request. Only a numbered-choice box, a [y/n] prompt, or an \"Enter to select\" menu is a permission/decision gate.");
        sb.AppendLine();
        ...
        sb.AppendLine("- \"... Searching... (4s)   [empty input box]   >   bypass permissions on (shift+tab to cycle) · esc to interrupt\" -> {\"state\":\"working\",\"reason\":\"the bottom footer contains esc-to-interrupt, so working even though an empty input box is also shown\",\"awaiting\":\"\"}");
        sb.AppendLine("- \"... * Running tests... (esc to interrupt)   [tests passed]   All 42 tests pass.   >   bypass permissions on (shift+tab to cycle)\" -> {\"state\":\"waiting_for_input\",\"reason\":\"the bottom footer has NO esc-to-interrupt; the one higher up is stale scrollback from the finished test run\",\"awaiting\":\"\"}");
        sb.AppendLine("- \"... Which joke do you want to hear?  > 1. Option 1  2. ...  Enter to select . up/down to navigate . Esc to cancel\" -> {\"state\":\"waiting_for_permission\",\"reason\":\"parked on an interactive Enter-to-select choice menu\",\"awaiting\":\"Which joke do you want to hear?\"}");
        sb.AppendLine("- \"... That will delete the old file. OK to proceed?   >   bypass permissions on (shift+tab to cycle)\" -> {\"state\":\"waiting_for_input\",\"reason\":\"prose question at an empty input box, no numbered box - waiting for the user's typed reply\",\"awaiting\":\"OK to proceed?\"}");
        sb.AppendLine("- \"####  x9x9  ????\" -> {\"state\":\"unknown\",\"reason\":\"garbled characters, no input box, spinner, or footer\",\"awaiting\":\"\"}");
        sb.AppendLine();
