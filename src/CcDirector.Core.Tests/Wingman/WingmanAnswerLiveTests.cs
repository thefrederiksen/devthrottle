...
public sealed class WingmanAnswerLiveTests
{
    ...
    [Fact]
    public async Task CleanVoiceTranscript_routes_hey_wingman_to_wingman_and_strips_phrase()
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("WINGMAN_LIVE_TESTS"), "1", StringComparison.Ordinal))
        {
            _out.WriteLine("Skipped: set WINGMAN_LIVE_TESTS=1 to run the live routing test.");
            return;
        }
        var claude = ResolveClaudePath();
        if (claude is null) { _out.WriteLine("Skipped: claude CLI not found."); return; }

        // 1. Addressed to the wingman -> target=wingman, wake phrase stripped.
        var wm = await WingmanService.CleanVoiceTranscriptAsync(
            "Hey wingman, read me the whole article we just wrote.", repoPath: "", claudeExePath: claude);
        _out.WriteLine($"[wingman case] target={wm.Target} cleaned=\"{wm.Cleaned}\"");
        Assert.Equal("wingman", wm.Target);
        Assert.DoesNotContain("wingman", wm.Cleaned, StringComparison.OrdinalIgnoreCase);

        // 2. An ordinary instruction to the agent -> target=agent, left intact.
        var ag = await WingmanService.CleanVoiceTranscriptAsync(
            "Um, can you fix the bug in the login flow please.", repoPath: "", claudeExePath: claude);
        _out.WriteLine($"[agent case] target={ag.Target} cleaned=\"{ag.Cleaned}\"");
        Assert.Equal("agent", ag.Target);
    }
