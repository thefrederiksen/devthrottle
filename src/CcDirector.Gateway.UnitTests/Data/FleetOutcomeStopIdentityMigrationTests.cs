...
public sealed class FleetOutcomeStopIdentityMigrationTests
{
    ...
    public void AddFleetOutcomeStopIdentity_FromEmpty_AddsTheColumnsAndKeepsARecordFiledBefore()
    {
        ...
        try
        {
            ...
            Assert.Equal("20260921081600_AddFactoryActivity", all[^1]); // the migrations that sort after it
            Assert.Equal("20260920052924_AddRaisedSessions", all[^2]);
            Assert.Equal("20260920021757_AddDiscoveredRepositories", all[^3]);
            Assert.Equal("20260918171353_AddWingmanNarrationCallTrace", all[^4]);
            Assert.Equal(SqliteUnderTest, all[^5]);
            ...
            Assert.Equal("20260921081600_AddFactoryActivity", context.Database.GetAppliedMigrations().Last());
    ...
    /// </summary>
    [Theory]
    [InlineData("sqlite", "20260921081600_AddFactoryActivity")]
    [InlineData("postgres", "20260921084515_AddFactoryActivity")]
    public void TheNewestMigrationsDesigner_IsDiscovered_AndCarriesTheCurrentModel(string provider, string id)
    {
        ...
        Assert.Equal("AddFactoryActivity", type!.Name);
