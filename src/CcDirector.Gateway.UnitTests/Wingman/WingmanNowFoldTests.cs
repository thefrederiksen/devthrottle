...
public sealed class WingmanNowFoldTests
{
    ...
    /// <summary>A stop he answered THROUGH AN OPTION: the verdict row carries both the moment and his words, and
    /// the session went back to work two seconds later.</summary>
        ...
        DateTime? answeredAt = null, DateTime? backToWorkAt = null)
    {
        ...
        return new AnsweredTurnVerdict(verdict, answeredAt ?? AnsweredAt, chosen);
    }
    ...
    /// <summary>
    /// A MOMENT WITHOUT THE WORDS IS NOT AN ANSWER TO SHOW. Every stop answered before this slice existed carries
    /// exactly that - a recorded moment and no record of what was chosen - and "You answered:" with nothing after
    /// the colon is worse than saying nothing at all.
    /// </summary>
    [Fact]
    public void A_recorded_answer_with_no_words_is_not_read_back_to_him()
    {
        var verdict = Verdict(TurnVerdictVocabulary.NeededYou);
    ...
    /// <summary>
    /// ANOTHER SESSION ANSWERING IS NOT HIM ANSWERING. The first reply after a stop is the answer to it, so a
    /// fleet message there means he has not answered yet - and the card must not tell him he has.
    ///
    /// It STOPS the search rather than skipping past: skipping would find a later message of his own and present
    /// it as the answer to a stop another session had already dealt with.
    /// </summary>
    [Fact]
    public void A_fleet_message_after_the_stop_is_never_read_back_as_his_own_answer()
    {
        var framed = FleetMessaging.BuildFramedMessage("22222222-2222-2222-2222-222222222222", "Architect",
            "SOREN_NORTH", "Merge it - the notes are reviewed.");

        var now = FoldWith(WorkingRow(), new[] { UnansweredStop() }, Asked(framed, AnsweredAt),
    ...
    {
        var verdict = Verdict(TurnVerdictVocabulary.NeededYou);
        var stop = new AnsweredTurnVerdict(verdict, AnsweredAt, "Allow the merge");
