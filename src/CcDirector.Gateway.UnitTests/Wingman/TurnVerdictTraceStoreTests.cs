...
public sealed class TurnVerdictTraceStoreTests : IDisposable
{
    ...
    [Fact]
    public void AJoinedStop_IsKeptWithItsOwnMoment_AndNoVerdictAndNoContent()
    {
        var store = new TurnVerdictTraceStore(_harness.Open());
        store.Append(TenantA, new TurnVerdictTrace
        {
            ...
            SessionId = "sid-1",
            RecordedAtUtc = RecordedAt,
            TurnEndObservedAtUtc = RecordedAt.AddMinutes(3),
            Trigger = "turn-end",
            Outcome = TurnVerdictTraceOutcomes.Joined,
            Cause = ActivityCauses.AlreadyJudging,
        });

        var read = Assert.Single(store.History(TenantA, "sid-1"));
        Assert.Equal(TurnVerdictTraceOutcomes.Joined, read.Outcome);
        Assert.Equal(RecordedAt.AddMinutes(3), read.TurnEndObservedAtUtc);
        Assert.Null(read.VerdictId);
        Assert.Null(read.Verdict);
        Assert.Null(read.Prompt);
    }
    ...
    [Fact]
    public void APackageWhoseTextIsOverTheCeiling_IsDroppedByTheCheapCount_WithoutBeingSerialized()
    {
        var huge = Trace().Package! with { ScreenRows = Enumerable.Repeat(new string('s', 1_000), 300).ToArray() };

        var bounded = TurnVerdictTraceStore.ApplyCeilings(Trace() with { Package = huge });

        Assert.Null(bounded.Package);
        Assert.True(bounded.PackageOmitted);
        Assert.True(TurnVerdictTraceStore.PackageCharacters(huge) > TurnVerdictTraceStore.MaxPackageJsonChars);
    }
