...
public sealed class RetiredMessagingWordsTests
{
    ...
    /// document the refusal rather than teach the command. Nothing else may, in the inventory or the tree.
    /// </summary>
    ...
    /// <summary>
    /// Directories that hold build output rather than repository text. "build" is here because a local
    /// Python build leaves a copy of the command line's own sources under
    /// <c>tools/cc-devthrottle/build/lib/</c>; <c>.gitignore</c> ignores <c>build/</c> and no tracked
    /// file lives under any directory of that name, so scanning it reports the same source twice and
    /// depends on whether somebody happened to run a build.
    /// </summary>
    private static readonly HashSet<string> TreeSkippedDirectories = new(StringComparer.Ordinal)
    {
        ".git", "bin", "obj", "build", "node_modules", "dist", ".venv", "venv", "__pycache__", ".pytest_cache", "TestResults",
    };
    ...
    public void Nothing_in_the_repository_outside_the_named_history_uses_the_retired_messaging_words()
    {
        ...
        {
            read++;
            offenders.AddRange(Scan(file, PhrasesFor(Relative(root, file), TreePhrases)));
        }
        ...
        Assert.True(offenders.Count == 0,
            ...
            "interrupts, one-line messages, naming any session as owner, or teaching " +
            "'cc-devthrottle session prompt'). Rewrite the text to the queue - see docs/FleetMessaging.md - " +
            "or, if it is genuinely a dated record, add it to TreeExemptions with the reason; a file that " +
            "documents the typing refusal rather than teaching it belongs in TypingCommandExemptions:\n  " +
            string.Join("\n  ", offenders));
    }
    ...
    public void The_tree_scan_reads_the_files_this_round_fixed_and_every_exemption_exists()
    {
        ...
        foreach (var (path, _) in TypingCommandExemptions)
            Assert.True(File.Exists(Path.Combine(root, path)),
                $"The typing-command exemption '{path}' names nothing in the repository; remove it.");
    }
