...
public sealed class WingmanVerdictBoundaryAuditTests
{
    ...
    {
        SettleWait, ScreenRead, ReuseCheck, ReattemptRefusal, ConversationRead, ProviderDeadline, AccountCeiling, ModelCall,
    };
    ...
    public void Every_read_count_the_charter_states_matches_where_the_code_reads()
    {
        ...
        Assert.True(checkedSteps >= 4,
            $"Only {checkedSteps} read count(s) were found in the charter's boundary block; expected the four it states " +
            "(NO for the first checks, ONE for the reuse check, ONE for the re-attempt refusal, TWO for the deadline and ceiling). " +
