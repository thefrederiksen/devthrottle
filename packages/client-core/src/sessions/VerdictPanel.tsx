...
// Gateway has saved one, otherwise the judge's summary; and "this is wrong", wired by slice G to the feedback route.
//
// THERE ARE NO ANSWER BUTTONS (the turn pipeline mission, phase 4; the owner, 25 September: "Drop the buttons for now.
// A menu is simply needs you, and the narration says open the session to choose"). A record stored under contract v3
// still carries its menu and options; the panel renders its label and words and does not offer them.
//
...
// a question, it was telling me it was done". What is still true about a superseded verdict is what it SAID, which
// is what a correction is about.
//
...
//
// THE CLIENT IS DUMB. Everything shown is the Gateway's stamp, verbatim, and a refusal from the feedback route is
// shown as it came, never swallowed and never reworded.
//
...
export function VerdictPanel({ sessionId, session, onReported }: VerdictPanelProps) {
  ...
  // A new stop is a new question: nothing reported carries over from the last one.
  useEffect(() => {
    setReporting(false);
    setCorrectWord("");
    setNote("");
  ...
  // What the row carries now, judged or refused, as against a record read out of the history.
