...
Take a finished change from this session to merged on origin/main.
  ...
  cc-ship respond <id> --fix|--keep|--drop [--note "the owner's words"]
      Record the owner's call on one finding. Relay his answer; never make it up.
  cc-ship status [--json]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = add("continue", "resume the run")
    p.add_argument("--owner-allows-another-round", action="store_true",
                   help="only when the owner said so after the fix-round limit")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "start":
            ...
            run = engine.resume(_current_run(cwd), args.owner_allows_another_round)
        ...
        data = {"state": "failed", **err.as_dict(), "next_step": err.help}
        _print(data, as_json)
        return err.exit_code
    except Exception as exc:  # the entry point: never a bare traceback for an agent to parse
        err = engine.as_ship_error(exc) if isinstance(exc, (engine.fleet.FleetError,
                                                             engine.preview.PreviewError)) else \
            ShipError("internal-error", f"{type(exc).__name__}: {exc}",
                      "This is a cc-ship defect. Run: cc-ship status, and report it with the run folder.")
        _print({"state": "failed", **err.as_dict(), "next_step": err.help}, as_json)
        return EXIT_ERROR
