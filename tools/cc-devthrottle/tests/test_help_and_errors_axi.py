...
def _default_answers():
    ...
    return {
        ...
        ("POST", "fleet/reply"): {"status": "queued", "messageId": "r1", "recipientSessionId": PARENT,
                                  "inReplyToMessageId": "q1", "correlationId": "c1"},
...
MUTATIONS = [
    ...
    ("message send --reply-wanted", ["message", "send", SID, "hello?", "--reply-wanted", "--reply-by", "15"], None,
     ["cc-devthrottle message inbox", "cc-devthrottle session list"]),
    ("message reply", ["message", "reply", "c1", "yes"], AS_SID,
     ["cc-devthrottle message inbox", 'cc-devthrottle session report "<what you did>"']),
    ("message inbox", ["message", "inbox"], AS_SID,
     ["cc-devthrottle message inbox --all", 'cc-devthrottle message reply <correlation-id> "<answer>"',
      'cc-devthrottle message send <id> "<message>"', 'cc-devthrottle session report "<what you did>"']),
...
ERRORS = [
    ...
    ("send reply-by without reply-wanted", ["message", "send", SID, "x", "--reply-by", "5"], None, None, USAGE,
     ["--reply-by", "--reply-wanted"]),
    ("send reply-wanted to all", ["message", "send", "all", "x", "--reply-wanted"], AS_SID, None, USAGE,
     ["--reply-wanted", "one session id, not 'all'"]),
    ("send reply-by not a number", ["message", "send", SID, "x", "--reply-wanted", "--reply-by", "soon"], None, None,
     USAGE, ["--reply-by"]),
    ("send reply-by refused", ["message", "send", SID, "x", "--reply-wanted", "--reply-by", "0"], None,
     _set(**{f"POST sessions/{SID}/message": _raise("replyByMinutes must be between 1 and 1440; 0 was given.", 400)}),
     FAILURE, ["Not queued:", "replyByMinutes must be between 1 and 1440; 0 was given.", "cc-devthrottle session list"]),
    ("reply blank id", ["message", "reply", " ", "x"], AS_SID, None, USAGE, ["id is blank", "message reply"]),
    ("reply blank text", ["message", "reply", "c1", " "], AS_SID, None, USAGE, ["reply is blank", "message reply"]),
    ("reply refused", ["message", "reply", "c1", "x"], AS_SID,
     _set(**{"POST fleet/reply": _raise("Only the session message q1 was sent to may reply to it. Nothing was queued.", 403)}),
     FAILURE, ["Not queued:", "the reply to c1 was not queued", "Only the session message q1 was sent to may reply to it",
               "cc-devthrottle message inbox --all", 'cc-devthrottle session report "<what you would have said>"']),
    ("reply refused in body", ["message", "reply", "c1", "x"], AS_SID,
     _set(**{"POST fleet/reply": {"status": "refused", "error": "one reply per question"}}), FAILURE,
     ["Not queued: one reply per question", "cc-devthrottle session list"]),
    ("reply no status", ["message", "reply", "c1", "x"], AS_SID, _set(**{"POST fleet/reply": {}}), FAILURE,
...
UNCONFIRMED = [
    ...
    ("reply empty", ["message", "reply", "c1", "yes"], AS_SID,
     _set(**{"POST fleet/reply": {}}), "did not accept"),
    ("reply queued with no id", ["message", "reply", "c1", "yes"], AS_SID,
     _set(**{"POST fleet/reply": {"status": "queued", "recipientSessionId": PARENT}}), "gave no reply id"),
...
SHORT_HELP_LATER = {"fleet-manager", "fleet", "factory"}
