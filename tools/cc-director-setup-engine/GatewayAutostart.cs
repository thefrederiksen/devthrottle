...
/// <summary>
/// The Gateway tray app's per-user autostart entry under
/// HKCU\Software\Microsoft\Windows\CurrentVersion\Run.
///
/// Per-user (HKCU) is the correct scope: the Gateway only does useful work while the user
/// is logged in (the whole fleet is logon-bound), so it starts in the user's interactive
/// session - never as a machine service (docs/plans/gateway-tray-app.md). Lives in the
/// engine so the installer, the uninstaller, and the tray app itself all agree on one
/// value name and command-line format.
/// </summary>
public static class GatewayAutostart
{
    private const string RunKeyPath = @"Software\Microsoft\Windows\CurrentVersion\Run";

    /// <summary>The Run-key value name.</summary>
    public const string ValueName = "CcDirectorGateway";
    ...
    public static bool EnsureRegistered(string exePath, string? arguments = null)
    {
        ...
        EngineLog.Write($"[GatewayAutostart] EnsureRegistered: {desired}");
        ...
        {
            EngineLog.Write("[GatewayAutostart] EnsureRegistered: already up to date");
            return false;
        }

        key.SetValue(ValueName, desired, RegistryValueKind.String);
        EngineLog.Write("[GatewayAutostart] EnsureRegistered: wrote Run-key value");
        return true;
    }
    ...
    {
        EngineLog.Write("[GatewayAutostart] Unregister");
        using var key = Registry.CurrentUser.OpenSubKey(RunKeyPath, writable: true);
